...
import type { SearchMode, SemanticSearchInput } from "./contracts.js";
...
function takeValue(args: string[], index: number, flag: string): string {
  ...
  if (!value || value.startsWith("-")) throw new SemanticSearchError("INVALID_INPUT", `${flag} requires a value`);
  return value;
}
...
async function main(): Promise<number> {
  ...
  const input: SemanticSearchInput = { query: "" };
  const roots: string[] = [];
  let pretty = false;
  ...
  let modelCacheDirectory: string | undefined;
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (flag === "--query" || flag === "-q") input.query = takeValue(args, index++, flag);
    else if (flag === "--root" || flag === "-r") roots.push(takeValue(args, index++, flag));
    else if (flag === "--limit" || flag === "-n") input.limit = Number(takeValue(args, index++, flag));
    else if (flag === "--mode" || flag === "-m") {
      const mode = takeValue(args, index++, flag);
      if (!(SEARCH_MODES as readonly string[]).includes(mode)) throw new SemanticSearchError("INVALID_INPUT", `Unknown mode: ${mode}`);
      input.mode = mode as SearchMode;
    } else if (flag === "--no-cache") input.noCache = true;
    else if (flag === "--rg-trace") input.rgTrace = true;
    else if (flag === "--pretty") pretty = true;
    else if (flag === "--python") pythonBin = takeValue(args, index++, flag);
    else if (flag === "--script") scriptPath = takeValue(args, index++, flag);
    else if (flag === "--cache-directory") cacheDirectory = takeValue(args, index++, flag);
    else if (flag === "--embedding-model") embeddingModelPath = takeValue(args, index++, flag);
    else if (flag === "--model-cache") modelCacheDirectory = takeValue(args, index++, flag);
    else if (flag === "--offline") offline = true;
    else if (flag === "--no-model-download") autoDownload = false;
    else if (flag === "--gpu-layers") gpuLayers = Number(takeValue(args, index++, flag));
    else if (flag === "--timeout-ms") timeoutMs = Number(takeValue(args, index++, flag));
    else throw new SemanticSearchError("INVALID_INPUT", `Unknown argument: ${flag}`);
  }
  if (roots.length) input.roots = roots;
