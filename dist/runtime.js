...
function normalizeInput(raw, cwd) {
    const query = typeof raw.query === "string" ? raw.query.trim() : "";
    if (!query)
        throw new SemanticSearchError("INVALID_INPUT", "query is required and must be a non-empty string");
...
export function extractSearchReadHits(hits, root = process.cwd()) {
    const out = [];
    const seen = new Set();
    for (const hit of hits) {
        const [start, end] = hit.lines;
        if (!hit.file || start < 1 || end < start)
            continue;
...
export function createSemanticSearchRuntime(options = {}) {
    ...
    const search = async (raw, context = {}) => {
        let normalized;
        try {
            ...
            if (options.cacheDirectory) {
                ...
                if (!cacheDirectory ||
                    ...
                    throw new SemanticSearchError("INVALID_INPUT", "cacheDirectory must be a non-empty path relative to each search root");
                }
            }
        }
        catch (error) {
            emit({ type: "search.failed", code: errorCode(error), message: error.message, at: now() }, context);
            throw error;
        }
        emit({
            type: "search.started",
        ...
        try {
            ...
            const callbacks = [options.onHits, context.onHits].filter((callback, index, all) => Boolean(callback) && all.indexOf(callback) === index);
...
export function createSemanticSearchTool(options = {}) {
