...
export function bundledScriptPath() {
    return fileURLToPath(new URL("../python/agentic_semantic_search.py", import.meta.url));
}
export async function resolvePythonBin(configured, env = process.env) {
...
function parseHits(stdout) {
    ...
    try {
        ...
        throw new SemanticSearchError("INVALID_OUTPUT", `Search engine returned invalid JSON: ${error.message}`, {
...
export async function runSearchProcess(input, options) {
    if (options.signal?.aborted)
        throw new SemanticSearchError("ABORTED", "Search was aborted before launch");
    try {
